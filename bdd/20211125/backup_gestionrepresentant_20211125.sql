...
--
-- Temporary table structure for view `vue`
--

DROP TABLE IF EXISTS `vue`;
/*!50001 DROP VIEW IF EXISTS `vue`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `vue` AS SELECT 
 1 AS `IdVente`,
 1 AS `IdRepres`,
 1 AS `IdProduit`,
 1 AS `IdClient`,
...
--
-- Final view structure for view `vue`
--
...
/*!50001 VIEW `vue` AS select `v`.`IdVente` AS `IdVente`,`v`.`IdRepres` AS `IdRepres`,`v`.`IdProduit` AS `IdProduit`,`v`.`IdClient` AS `IdClient`,`v`.`Quantite` AS `Quantite`,`c`.`NomClient` AS `NomClient`,`c`.`VilleClient` AS `VilleClient` from (`ventes` `v` join `clients` `c` on((`c`.`IdClient` = `v`.`IdClient`))) */;
...
-- Dump completed on 2021-11-25 12:20:01
