...
//on envoi la requête
req.open('GET', 'https://opendata.lillemetropole.fr/api/records/1.0/search/?dataset=ilevia-gammetarifaire&q=&facet=prix&facet=typetittreproduit&facet=libelle', true);
req.send(null);
    ...
    if (this.readyState === XMLHttpRequest.DONE) {
        if (this.status === 200) {
            ...
            for (let i = 0; i < enregs.length; i++) {
                ...
                ligne.id = i;
                typetitre = document.createElement("div");
                typetitre.setAttribute("class", "typetitre");
                ligne.appendChild(typetitre);
                ...
                typetitre.innerHTML = enregs[i].fields.typetitreproduit;
                prix.innerHTML = enregs[i].fields.prix;
